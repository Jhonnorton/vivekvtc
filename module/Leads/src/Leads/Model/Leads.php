    ...
    public function getCollection($startdate = null, $enddate = null, $page = false) {
        ...
        foreach ($collection as $item) {
            ...
            $newCollection[$item->getId()] = array(
                ...
                'hotelData' => $this->_getHotelData($item->getTypename(), $item->getType()),
                'typeid' => $item->getType(),
                ...
                'no_of_persons' => $item->getNoOfPersons(),
                'lead' => $item,
                    //'roomid'=>$item->getRoomId(),
            );
        }
...
//d($type);
    ...
    protected function _getHotelData($resortid, $type) {
...
//         d($object);     
        ...
        $entity->setNoOfPersons($object['persons']);
...
//                    d($object);
                    for ($i = 0; $i < $rooms; $i++) {
                        if ($object['roomCategory'][$i]) {
                            ...
                            $entity1->setNetCost($object['netcost'][$i]);
                            $date = getdate();
                            //d();
                            // d($files);
                            if($files['image'][$i]['name']){
                                $entity1->setImage($date[0] . '-' . $files['image'][$i]['name']);
                            }else{
                                $entity1->setImage($object['path'][$i]);
                            }
                            ...
                            if(!empty($files['image'][$i]['tmp_name'])){
                                $this->saveImages($files['image'][$i]['tmp_name'], $date[0] . '-' . $files['image'][$i]['name']);
                            }
                        }
                    }
                }
...
//             d($countmax);
            ...
            if ((int) $countmax > 0) {
                for ($i = 0; $i < $countmax; $i++) {
                    ...
                    $lead = $em->find('Base\Entity\InventoryLeads', (int) $inventoryLeadId->getId());
//                    echo $lead->getId()."--";
                    $entity2->setInventoryLeadId($lead);
...
//                        $transfers = $this->getEntityManager()->getRepository('\Base\Entity\InventoryTransfer')->findOneBy(array('id' => $object['transfers'][$i]));
    ...
    public function getInventoryLead($leadId){
        $em = $this->getEntityManager();
        $item = $em->getRepository('\Base\Entity\InventoryLeads')->findOneBy(array('leadId' => $leadId));
