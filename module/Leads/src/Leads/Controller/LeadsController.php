...
//            d($data);
            ...
            if ($this->model->isValidModel($form) && $this->model->isValidImage($form, $files)) {
                ...
                $inventoryLeadId=$this->model->quotesave($object);

//                d($inventoryLeadId->getId());
...
//                 $data_url = urlencode(json_encode($data_url));
                $dateenc=strtr(base64_encode(date('Y-m-d H:i:s')), '+/=', '-_,');
                 $url="http://".$_SERVER['HTTP_HOST']."/quoteReservation/".$object['object']['id']."/".$inventoryLeadId->getId()."/".$dateenc;
//                 d($url);
...
//                $this->model->sendquote($to, $clientname, array($leads), array('link'=>'<a href="'.$url.'">Link to Reservation</a>'), $subject);
    ...
    public function quoteReservationAction(){
        ...
        $inventorylead = $this->model->getInventoryLead($leadId);
//        d($lead);
        $data = array('leadId' => $leadId,'lead'=>$lead,'inventoryLead'=>$inventorylead);
