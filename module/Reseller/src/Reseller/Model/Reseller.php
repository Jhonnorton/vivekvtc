...
//    public static $imagesBaseUrl = 'http://192.155.246.146:9084/app/webroot/img/user_uploads/'; //Development Staging
    ...
    public function resortsave($object){
        ...
        //d($obj);
        $em = $this->getEntityManager();
        $entity = $em->find('\Base\Entity\Avp\Resorts', (int) $obj['id']);
        $date = getdate();

        $entity->setTitle($obj['title']);
        $entity->setPageHeading($obj['pageHeading']);
        $entity->setAmenities($obj['amenities']);
        $entity->setOverview($obj['overview']);
        $entity->setCategoryId($obj['categoryId']);
        
        $entity->setEntertainment($obj['entertainment']);
        $entity->setPopoverTitle($obj['popoverTitle']);
        $entity->setPopoverContent($obj['popoverContent']);
        $entity->setMetaTitle($obj['metaTitle']);
        $entity->setMetaDescription($obj['metaDescription']);
        $entity->setMetaKeywords($obj['metaKeywords']);
...
//        // d('success');
    ...
    public function update($object, $data) {
        ...
        $em = $this->getEntityManager();
//        d($data);
        $u = $em->find('\Base\Entity\Users', (int) $data['id']);
        if (!$this->isValidMd5($data['password'])) {
            $u->setPassword(md5($data['password']));
            $em->persist($u);
            $em->flush();
        }
        
//        if (!$this->isValidMd5($object->getUserId()->getPassword())) {
//
//            $u->setPassword(md5($object->getUserId()->getPassword()));
//        }
////        d($u);
//        $user = parent::save($u);
        ...
        $em = $this->getEntityManager();
//        d($data['affiliateid']);
        $entity = $this->getEntityManager()->getRepository('\Base\Entity\Avp\Affiliates')->findOneBy(array('id' => $data['affiliateid']));
//        $entity = $em->find('\Base\Entity\Avp\Affiliates', (int) $data['affiliateid']);
        $user = $em->find('\Base\Entity\Users', (int) $data['id']);
//         d($entity);
        $entity->setUserId($user);
        $entity->setAffiliatePackageId(0);
    ...
    public function getEventRooms($eventid = null) {
        ...
        $em = $this->getEntityManager();
        $collection = $em->getRepository('Base\Entity\Avp\EventRooms')->findBy(array('eventId' => $eventid,'isDeleted'=>0));
    ...
    public function getResortRooms($resortid = null) {

        $em = $this->getEntityManager();
        $collection = $em->getRepository('Base\Entity\Avp\ResortRooms')->findBy(array('resortId' => $resortid,'isDeleted'=>0));
    ...
    public function eventroomsupdate($object) {
      ...
      public function deleterooms($id) {
        ...
        $eventRooms = $em->find('\Base\Entity\Avp\EventRooms',$id);
        if($eventRooms){
            $eventRooms->setIsDeleted(1);
            $em->persist($eventRooms);
            $em->flush();
        }
...
//d('success');
    ...
    public function deleteevent($id) {
         ...
         */
            ...
            $entity = $em->getReference('\Base\Entity\Avp\Events', (int)$id);
    ...
    public function deleteResort($id){
            $em = $this->_entityManager;        
            $entity = $em->getReference('\Base\Entity\Avp\Resorts', (int)$id);
                ...
                foreach($associatedEvents as $rowEvents){
                    if($rowEvents->getResortId() == $id){
                        ...
                        foreach($eventRoomEntity as $eEntity){
                            ...
                            $reservationEntity = $em->getRepository('\Base\Entity\Avp\Reservation')->findBy(array('roomId'=> $eEntity->getId(),'type'=>2));
                            foreach($reservationEntity as $resEntity){
                            if (!empty($resEntity)) {
                                $resEntity->setIsDeleted(1);
                                $em->persist($resEntity);
                        ...
                        $query1 = 'DELETE FROM vouchers WHERE link_to_type=2 AND link_to_type_name  =' . $ids;
                        $stmt = $em->getConnection()->prepare($query1);
                        $stmt->execute();
                        ...
                        $query1 = 'DELETE FROM vouchers WHERE link_to_type=1 AND link_to_type_name  =' . $id;
                        $stmt = $em->getConnection()->prepare($query1);
                        $stmt->execute();
    }
    
    
    public function deleteresortrooms($id){
            $em = $this->_entityManager;        
            $entity = $em->getReference('\Base\Entity\Avp\ResortRooms', (int)$id);
...
//        $collection = $query->getResult();
            ...
            $em = $this->_entityManager;        
            $entity = $em->getReference('\Base\Entity\Avp\Cruises', (int)$id);
