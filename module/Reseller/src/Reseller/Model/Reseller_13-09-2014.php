...
//    public static $imagesBaseUrl = 'http://192.155.246.146:9084/app/webroot/img/user_uploads/'; //Development Staging
    ...
    public function save($object, $data) {
        ...
        $name = isset($data['urlExtension']) ? $data['urlExtension'] : $data['firstName'];
        if ($name) {
            $entity->setName($name);
        }
...
//        d($newcollection);
    ...
    public function updatePaymentStatus($affiliate,$response){
        ...
        $reservation->setTransactionId($response['ACK']['TRANSACTIONID']);
...
//            $newcollection[$i]['commission']=0;
            foreach($commcollection as $comm){
                ...
                if($comm->getType()==1){
                    $newcollection[$i]['commission']+=($comm->getAmount()/100)*$reservation->getFinalCost();
                }else{
