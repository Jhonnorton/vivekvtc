...
//        d($this->_entity);
    ...
    public function resorteditAction() {

        $data = $this->getEm()->find('Base\Entity\Avp\Resorts',$this->_id);
        $form = new \Reseller\Form\ResortsForm($this->getEm(), $data);
        ...
        if ($this->request->isPost()) {
			...
			//if ($form->isValid ()) {
				if($this->model->isValidModel($form)){
                                        ...
                                        $this->addPageMessage('The resort has been updated.','success');				
					$this->redirect()->toRoute('affiliate-resort');
				}
			//} else {
				//$form->getMessages();
                               // d($form->getMessages());
    ...
    public function resortroomsAction() {
      ...
      //  d($this->_id);
        $data = $this->getEm()->find('Base\Entity\Avp\Resorts', $this->_id);
        $data = array('collection' => $this->model->getResortRooms($data->getId()), 'id' => $data->getId(),'imagePath'=>$this->model->getImagePath());
        return $this->getView($data);
    }
    ...
    public function resortviewAction() {
    ...
    public function resortDeleteAction() {
        ...
        $data = $this->getEm()->find('Base\Entity\Avp\Resorts', $this->_id);
        //d($data);
        $this->model->deleteResort($this->_id);

        $this->addPageMessage('The resort has been deleted.', 'success');
        $this->redirect()->toRoute('affiliate-resort');
    }
    
    public function resortroomsdeleteAction() {
        
        $data = $this->getEm()->find('\Base\Entity\Avp\ResortRooms', $this->_id);
        //d($data);
        $this->model->deleteresortrooms($this->_id);

        $this->addPageMessage('The resort rooms has been deleted.', 'success');
        $this->redirect()->toRoute('affiliate-resort-rooms', array('id' => $data->getResortId()));
    }
