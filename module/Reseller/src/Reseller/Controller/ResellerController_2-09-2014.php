            ...
            if ($form->isValid()) {
                if ($this->model->isValidModel($form)) {
                   ...
                   $affiliate=$this->model->save($form->getData(), $data);
                    ...
                    switch ($type) {
                        case 1:
                            $returnUrl = "http://".$_SERVER['HTTP_HOST']."/addaffiliate/express-response";
                            $currency = 'USD';
                            $cancelUrl = "http://".$_SERVER['HTTP_HOST']."/addaffiliate/express-response";
...
//                            d($response);
                            if ($response['ACK']['ACK'] == 'Success'){
                                ...
                                $data->updatePaymentStatus($affiliate);
    ...
    public function paymentResponseAction(){
        ...
        if ($response['ACK']['ACK'] == 'Success'){
            $data = $this->getModel('Reseller');
            $data->updatePaymentStatus($reservation->reservation);
