...
class Front extends \Base\Model\BaseModel {
    ...
    protected function _getAllRooms($type, $typeid){
        switch($type){
            case 1:
                $collection = $this->getEntityManager()->getRepository('\Base\Entity\Avp\ResortRooms')->findBy(array('resortId' => $typeid,'isDeleted'=>0));
                break;
            case 2:
                $collection = $this->getEntityManager()->getRepository('\Base\Entity\Avp\EventRooms')->findBy(array('eventId' => $typeid,'isDeleted'=>0));
                break;
            case 3:
                $collection = $this->getEntityManager()->getRepository('\Base\Entity\Avp\CruiseCabins')->findBy(array('cruiseId' => $typeid,'isDeleted'=>0));
                break;
        }
        return $collection;
    }
...
//                break;
//        }
        $reservation = $this->_saveStep3($object,$affiliateId);
        //  d("success");
    ...
    public function deleteAllReservationData($e,$object){
            $em = $this->getEntityManager();
            $rsm = new ResultSetMapping();
            //delete travellers data
            //d($e);
            $id = (int)$object['reservationId'];
            $t = (int)$object['type'];
...
//        $returnItem = $model->getItem($data[$key]);
    ...
    public function _saveStep1($data,$affiliateId){
        $em = $this->getEntityManager();
        if($data['reservationId']):
            $reservation=$em->find('Base\Entity\Reservation', (int) $data['reservationId']);    
            $this->deleteAllReservationData($reservation, $data);
        endif;
        $travellers = $this->_setClientsObject($data);
        $room = $this->_setSellItem($data);


        $reservation = $this->_setReservation($data,$affiliateId);
        //d($reservation);
        
        $this->setSessionData('Room', 'room', $room);
        $this->setSessionData('ReservationObject', 'reservationobject', $data);
    ...
    public function _saveStep2($data,$affiliateId){
        $em = $this->getEntityManager();
        
         //if($data['reservationId']):
             $reservation = $this->_saveStep1($data,$affiliateId);
        //endif;
        
        $items = $this->_setItems($data);

        $excursions = $this->_setExcursions($data);
        $transfers = $this->_setTransfers($data);
    ...
    public function _saveStep3($data,$affiliateId){
        $em = $this->getEntityManager();

         if($data['reservationId']):
                $reservation = $this->_saveStep2($data,$affiliateId);
        endif;

            if($reservation):
                if ($data['paymentType'] == 0)://if payment type is deposit
                  $this->_setReservationPaymentDue($reservation, $data); //set up multiple payment dues
              endif;
            endif;
            return $reservation;
...
//        d($collection);
    ...
    public function getCollectionById($id){
        $em = $this->getEntityManager();
         ...
         $item = $em->find('Base\Entity\Reservation', (int) $id);
        $newCollection = array();
...
//        $collection = $em->getRepository('\Base\Entity\ReservationPaymentDue')->findBy(array('status' => 0));
        ...
        $qb = $em->createQueryBuilder();
        $qb->select('c')->from('\Base\Entity\ReservationPaymentDue', 'c')
                ->innerJoin('\Base\Entity\Reservation', 'r', \Doctrine\ORM\Query\Expr\Join::INNER_JOIN, 'r.id = c.reservationId')
                ->where('c.status=0')
                ->where('r.isBooked=1');
...
//        $id = $reservation->reservation;
    ...
    public function sendmailreminder($to, $id, array $params = null) {
        // $data = $this->getItemView($id);
        // if ($data) {
        return SendmailController::sendMailOnReminder($this->_serviceManager, $to, $id, $params, ' Payment Reminder ');
        // }
        return false;
    }
...
//        d($date);
      ...
      public function _getRoomDetails($roomId,$type) {
       ...
       public function _calculateAffiliateCommission($object,$affiliateId){
        ...
        switch($type){
            case 1:
                ...
                $qb->select('ip')
                    ...
                    ->innerJoin('\Base\Entity\Avp\Commissions', 'c', \Doctrine\ORM\Query\Expr\Join::INNER_JOIN, 'ip.commissionId = c.id')
                    ->innerJoin('\Base\Entity\Avp\ResortCommissions', 'ec', \Doctrine\ORM\Query\Expr\Join::INNER_JOIN, 'c.id = ec.commissionId')
                    ->where("ip.affiliateId=".$affiliateId)
                    ->andWhere("ec.resortId=".$typeId);
                $query = $qb->getQuery();
            ...
            case 2:
                ...
                $qb->select('ip')
                    ->from('\Base\Entity\Avp\AffiliateCommissionRel', 'ip')
                    ->innerJoin('\Base\Entity\Avp\Commissions', 'c', \Doctrine\ORM\Query\Expr\Join::INNER_JOIN, 'ip.commissionId = c.id')
                    ->innerJoin('\Base\Entity\Avp\EventCommissions', 'ec', \Doctrine\ORM\Query\Expr\Join::INNER_JOIN, 'c.id = ec.commissionId')
                    ->where("ip.affiliateId=".$affiliateId)
                    ->andWhere("ec.eventId=".$typeId);
                $query = $qb->getQuery();
...
//        $collection = $query->getResult();
        ...
        if ($collection) {
            ...
            foreach ($collection as $rel) {
                ...
                switch ($commission->getCommissionFor()) {
                    case 1:
                        $resortCommission = $em->getRepository('\Base\Entity\Avp\ResortCommissions')->findOneBy(array('commissionId' => $rel->getCommissionId(),'resortId'=>$typeId));
                        if ($resortCommission) {
                        ...
                        break;
                    case 2:
                        $eventCommission = $em->getRepository('\Base\Entity\Avp\EventCommissions')->findOneBy(array('commissionId' => $rel->getCommissionId(),'eventId'=>$typeId));
                        if ($eventCommission) {
           ...
           // echo "else";
            $commission = $em->getRepository('\Base\Entity\Avp\Commissions')->findBy(array('general' => '1'),array('id' => 'DESC'));
           // d($commission);
          //  $row=$commission;
            foreach($commission as $row){
              switch ($row->getCommissionFor()) {
                  case 1:
                      $resortCommission = $em->getRepository('\Base\Entity\Avp\ResortCommissions')->findOneBy(array('commissionId' => $row->getId(),'resortId'=>$typeId));
                        if ($resortCommission) {
                            if ($resortCommission->getResortId() == $typeId) {
                  ...
                  case 2:
                       ...
                       $eventCommission = $em->getRepository('\Base\Entity\Avp\EventCommissions')->findOneBy(array('commissionId' => $row->getId(),'eventId'=>$typeId));
                     //d($eventCommission);
                      if ($eventCommission) {
                            if ($eventCommission->getEventId() == $typeId) {
                                if ($row->getType() == "fixed") {
       ...
       // d($amount);
        if(!empty($amount)){
            $dueDate=date('Y-m-d', strtotime('+15 days', strtotime($object['travelFrom'])));  
            ...
            $entity->setDueDate(new \DateTime($dueDate));
...
//       $totalperson=$male+$female;
  ...
  public function checkOccupancy($post,$count){
    ...
    public function getResellerData($affiliateId){
        $em = $this->getEntityManager();
        $qb = $em->createQueryBuilder();
        $qb->select('ip')
            ->from('\Base\Entity\Avp\Affiliates', 'ip')
            ->where("ip.id=".$affiliateId);
        $query = $qb->getQuery();
	...
	public function getNotoficationData($user_id,$setting_name){
		$em = $this->getEntityManager();
		$em->flush();
        $qb = $em->createQueryBuilder();
        $qb->select('n')
            ->from('\Base\Entity\Avp\Notifications', 'n')
            ->where("n.user_id='".$user_id."' AND n.setting_name='".$setting_name."' AND n.status=1");
        $query = $qb->getQuery();
