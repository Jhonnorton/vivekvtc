...
class Items extends \Base\Model\BaseModel implements InputFilterAwareInterface{
    ...
    protected function isValidName($form){
        if($this->checkItem(array('name'=>$form->getData()->getName()))){
            $form->get('name')->setMessages(array('This item already exists'));
            return false;
        }
        return true;
    ...
    public function update($object) {              
        if($object->getLinkedTo() < self::LINKED_TO_RESORT_NAME){
            ...
            if($object->getLinkedTo() != self::LINKED_TO_EVENT_CATEGORY){
                $object->setEventCategoryId(null);
            }
        }
        parent::save($object);
    ...
    public function getAvpEm(){
        
        return $this->_serviceManager->get('doctrine.entitymanager.orm_avp');
        
    }
    
    public static function getItemsLinkedTo(ServiceLocatorInterface $serviceManager, $typeCode = 0, $id = 0){
        $items = array();
        $collection = array();       
        $model = $serviceManager->get('Items');
        if(!isset($model)) {
            return $items;
        }
        switch((int)$typeCode ){            
    ...
    public function getReservationItems($id) {
        ...
        $reservation = $em->getRepository('\Base\Entity\ReservationItem')->findBy(array('item' => $id));
        return $reservation;
    }
}