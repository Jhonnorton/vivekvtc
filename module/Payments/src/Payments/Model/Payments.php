...
class Payments extends \Base\Model\BaseModel implements InputFilterAwareInterface {
    ...
    /**
     * saveRefundAmount Process the refund amount into calculation
     * 
     * @param Array $data
     * @return Object
     */
    public function saveRefundAmount($data){
        $em = $this->getEntityManager();
        $reservation = new $this->_targetEntity;
        if (isset($data['id']) && (int) $data['id'] != 0) {
            $reservation = $em->find('Base\Entity\Reservation', (int) $data['id']);
        }
        if ($data['refundAmount'] != '' && $data['refundAmount'] > 0.00) {
//                 $reservation->setIsRefund(1);
                $reservation->setRefundAmt($data['refundAmount']);
                if(isset($data['depositAmoun']) && is_numeric($data['refundAmount'])) {
                   $reservation->setDepositAmoun($data['depositAmoun'] - $data['refundAmount']);
                }
        }
