...
class Payments extends \Base\Model\BaseModel implements InputFilterAwareInterface {
    ...
    protected function _getItemName($reservationId, $type) {
        ...
        switch ($type) {
            case self::RESORT_ROOM:
                ...
                $name = !is_null($item) ? $item->getRoom()->getRoomCategory() : null;
                break;
 ...
 //echo "in modle payment"; die;
    ...
    public function updatePaymentStatus($reservation, $response, $allvalue = null) {

        $reservation = $this->_setReservation($reservation, $response);
    ...
    protected function _setPaymentDue($data, $reservation) {
        $due = count($data);
        if ((int) $due > 0) {
            if ($due == 1) {
    ...
    protected function _setReservation($data, $response) {
    ...
    public function updateReservationPaymentStatus($data, $object, $allvalue) {
        ...
        $this->_setPaymentDue($paymentdue, $data);
