...
//        }
        ...
        public function delete($id){
            ...
            foreach($item as $row){
                ...
                $promoEntity = $this->_serviceManager->get('doctrine.entitymanager.orm_default')->getRepository('\Base\Entity\CruisePromo')->findBy(array('cruiseId'=> $id));
                        
                foreach($promoEntity as $pEntity){
                    if (!empty($pEntity)) {
                        ...
                        $query1 = 'DELETE FROM cruise_promo WHERE cruise_id  =' . $id;
                        $stmt = $em->getConnection()->prepare($query1);
                        $stmt->execute();
