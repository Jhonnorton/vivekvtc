    ...
    public function saveEventtRooms($data,$eventId){
        ...
        if(!empty($delArr)){
         foreach($delArr as $delId){
            $query1 = 'DELETE FROM reservation WHERE room_id =' . $delId['id'];
            $stmt = $em->getConnection()->prepare($query1);
            $stmt->execute();

            $query1 = 'DELETE FROM reservation_event_room WHERE event_room_id =' . $delId['id'];
            $stmt = $em->getConnection()->prepare($query1);
            $stmt->execute();
            
            $em = $this->getEntityManager();
            $query1 = 'DELETE FROM inventory_event WHERE room_id =' . $delId['room_id'] . ' AND event_id ='.$eventId;
            $stmt = $em->getConnection()->prepare($query1);
            $stmt->execute();
