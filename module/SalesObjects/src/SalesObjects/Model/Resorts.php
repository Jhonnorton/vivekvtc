            ...
            foreach($item as $row){
                ...
                foreach($associatedEvents as $rowEvents){
                    if($rowEvents->getResortId() == $id){
                        ...
                        $qb = $this->_serviceManager->get('doctrine.entitymanager.orm_default')->createQueryBuilder();                
                        $qb->select('e')
                            ->from('\Base\Entity\EventsPromo', 'e')
                            ->where('e.eventId = '.$ids);
                        $query = $qb->getQuery();
                        $results = $query->getResult();
                        
                        foreach($results as $pEntity){
                            if (!empty($pEntity)) {
                                $query1 = 'DELETE FROM inventory_promo WHERE id  =' . $pEntity->getId();
                                $stmt = $em->getConnection()->prepare($query1);
                                $stmt->execute();
                            }
                        }
                        
                                $query1 = 'DELETE FROM events_promo WHERE event_id  =' . $ids;
                                $stmt = $em->getConnection()->prepare($query1);
                                $stmt->execute();
                        ...
                        //$promoEntity = $em->getRepository('\Base\Entity\Avp\ResortPromo')->findBy(array('resortId'=> $id));
                        $em = $this->_serviceManager->get('doctrine.entitymanager.orm_default');
                        $qb = $em->createQueryBuilder();                
                        $qb->select('e')
                            ->from('\Base\Entity\ResortPromo', 'e')
                            ->where('e.resortId = '.$id);
                        $query = $qb->getQuery();
                        $results = $query->getResult();
                        //echo "<pre>"; 
                        ...
                        foreach($results as $pEntity){
                            ...
                            if (!empty($pEntity)) {
                                $query1 = 'DELETE FROM inventory_promo WHERE id  =' . $pEntity->getPromo()->getId();
                                $stmt = $em->getConnection()->prepare($query1);
                                $stmt->execute();
                            }
                        }
                        
                                $query1 = 'DELETE FROM resort_promo WHERE resort_id  =' . $id;
                                $stmt = $em->getConnection()->prepare($query1);
                                $stmt->execute();
        }
