...
class EventRooms extends \Base\Model\AvpModel implements InputFilterAwareInterface{
	...
	public function isValidModelOnEdit($object){
        ...
        public function getEventRoomsCollection($sort = false){    	
            $order_by = 'ASC';
            if ($sort) {
                $order_by = 'DESC';
            }
            $em = $this->_serviceManager->get('doctrine.entitymanager.orm_avp');
            $query = $em->createQuery('SELECT er.id as EventRoomId,e.title as eventTitle,er.roomId AS roomId, rr.title AS roomTitle, rr.image AS roomImage, rr.price As roomPrice, rr.inStock As roomInStock,  r.title AS resortTitle, r.id AS resortId,e.id AS eventId'.
                    ' FROM \Base\Entity\Avp\EventRooms er,\Base\Entity\Avp\ResortRooms rr, \Base\Entity\Avp\Resorts r,\Base\Entity\Avp\Events e'.
                    ' WHERE er.roomId=rr.id AND rr.resortId=r.id AND er.eventId = e.id ORDER BY er.id '.$order_by);            
