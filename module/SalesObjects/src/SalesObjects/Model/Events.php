...
//        }
        ...
        public function delete($id){
            ...
            foreach($item as $row){
                ...
                $promoEntity = $this->_serviceManager->get('doctrine.entitymanager.orm_default')->getRepository('\Base\Entity\EventsPromo')->findBy(array('eventId'=> $id));
                ...
                foreach($promoEntity as $pEntity){
                    if (!empty($pEntity)) {
                        ...
                        $query1 = 'DELETE FROM events_promo WHERE event_id  =' . $id;
                        $stmt = $em->getConnection()->prepare($query1);
                        $stmt->execute();
