...
//d($data);
        ...
        if($data['remainingAmount'] <= 0 && $source == 1){
         $reservation->setPaymentType(1);
        }else{
            ...
            case self::EVENT_ROOM:
                ...
                if ($data['from']):
                    $qb->andWhere('ie.dateFrom <= :startDate')
                            ->setParameter("startDate", $data['from']);
//                    $qb->andWhere('ie.dateFrom BETWEEN :start AND :end')
//                        ->setParameter('start', $data['from'])
//                        ->setParameter('end', $data['to']);
                endif;
...
//                endif;
    ...
    protected function getRoomsCountAvailable($roomId, $start, $end, $type) {
        ...
        WHERE reservation_resort_room.room_id = $roomId AND reservation.is_booked = 1
        AND '$start' BETWEEN reservation.date_from
        AND reservation.date_to
        OR reservation.date_to
        BETWEEN '$start'
        AND '$end' AND reservation_resort_room.room_id = $roomId AND reservation.is_booked = 1
        OR '$end'
        BETWEEN reservation.date_from
        AND reservation.date_to AND reservation_resort_room.room_id = $roomId AND reservation.is_booked = 1
        OR reservation.date_from
        BETWEEN '$start'
        AND '$end'
        AND reservation_resort_room.room_id = $roomId AND reservation.is_booked = 1", $rsm);
                break;
        ...
        WHERE reservation_event_room.event_room_id = $roomId AND reservation.is_booked = 1
        AND '$start' BETWEEN reservation.date_from
        AND reservation.date_to
        OR reservation.date_to
        BETWEEN '$start'
        AND '$end' AND reservation_event_room.event_room_id = $roomId AND reservation.is_booked = 1
        OR '$end'
        BETWEEN reservation.date_from
        AND reservation.date_to AND reservation_event_room.event_room_id = $roomId AND reservation.is_booked = 1
        OR reservation.date_from
        BETWEEN '$start'
        AND '$end'
        AND reservation_event_room.event_room_id = $roomId AND reservation.is_booked = 1", $rsm);
                break;
        ...
        WHERE reservation_cruise_cabin.cabin_id = $roomId AND reservation.is_booked = 1
        AND '$start' BETWEEN reservation.date_from
        AND reservation.date_to
        OR reservation.date_to
        BETWEEN '$start'
        AND '$end' AND reservation_cruise_cabin.cabin_id = $roomId AND reservation.is_booked = 1
        OR '$end'
        BETWEEN reservation.date_from
        AND reservation.date_to AND reservation_cruise_cabin.cabin_id = $roomId AND reservation.is_booked = 1
        OR reservation.date_from
        BETWEEN '$start'
        AND '$end'
        AND reservation_cruise_cabin.cabin_id = $roomId AND reservation.is_booked = 1", $rsm);
                break;
        }
    ...
    public function getCollectionPending($startdate = null, $enddate = null, $page = false) {
        ...
        if (!empty($startdate) || !empty($enddate)) {
           ...
           If (!empty($startdate) && !empty($enddate)) {
            ...
            $qb->select('c')->from('\Base\Entity\Reservation', 'c')
                    ->where($qury)
                    ->andwhere('c.isBooked=0 AND c.status!=3');
                   // ->setParameter('id', $startdate)
                   // ->setParameter('endid', $enddate);
            ...
            $qb = $em->createQueryBuilder();
            $qb->select('c')->from('\Base\Entity\Reservation', 'c')
                    ->where('c.isBooked=0 AND c.status!=3');
