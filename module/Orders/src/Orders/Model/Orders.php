...
class Orders extends \Base\Model\BaseModel implements InputFilterAwareInterface {
    ...
    public function getCollection($startdate = null, $enddate = null, $page = false, $type = null) {
        ...
        if (!empty($startdate) || !empty($enddate)) {
            ...
            If (!empty($startdate) && !empty($enddate)) {
                $qury = "c.createdAt >= '$startdate' AND c.createdAt <= '$enddate' AND c.isBooked=1 AND c.isDeleted = 0";
            } else if (!empty($startdate) && empty($enddate)) {

                $qury = "c.createdAt >= '$startdate' AND c.isBooked=$isBooked AND c.isDeleted = 0";
            } else if (empty($startdate) && !empty($enddate)) {

                $qury = "c.createdAt <= '$enddate' AND c.isBooked=$isBooked AND c.isDeleted = 0";
            }
            ...
            $qb->select('c')->from('\Base\Entity\Reservation', 'c')
                    ->where("c.isBooked=$isBooked AND c.isDeleted = 0")
                    ->orderBy('c.id', 'DESC');
            $query = $qb->getQuery();
            $collection = $query->getResult();
        }
...
//d($data);
        ...
        if ($source == 1) {
            if($data['discount'] > 0){
                if($data['discountType']==0){
                    $dis=($data['discount']/100)*$data['totalCost'];
                }else{
                    $dis=$data['discount'];
                }
                $reservation->setAppliDiscount($data['discount'] + $data['olddiscount']);
            }else{
                $reservation->setAppliDiscount($data['olddiscount']);
            }
        } else {
        ...
        if ($source == 1) {
            ...
            if ($data['refund'] != '' && $data['refund'] > 0.00) {
...
//                    $qb->andWhere('ih.dateFrom >= :startDate')
//                            ->setParameter("startDate", $data['from']);
//                    $qb->andWhere('ih.dateTo >= :startDate')
//                            ->setParameter("startDate", $data['from']);
//                    $qb->andWhere('ih.dateFrom <= :endDate')
//                            ->setParameter("endDate", $data['to']);
//
//                endif;
...
//                d($noDays);
                $qb->select('ie')
                        ...
                        ->leftJoin('ie.eventId','e');
                        -//>innerJoin('\Base\Entity\Avp\Events', 'c', \Doctrine\ORM\Query\Expr\Join::INNER_JOIN, 'ie.eventId = c.id');
//                        ->leftJoin('ie.roomId', 'er')
                ...
                $qb->andWhere('e.isDeleted = :isDeleted')
                        ->setParameter("isDeleted", 0);
                $qb->andWhere('e.approved = :approved')
...
//                            ->setParameter("startDate", $data['from']);
                    ...
                    $qb->andWhere('ie.dateFrom <= :endDate')
                            ->setParameter("endDate", $data['to']);
//                    $qb->andWhere('c.minimumStayDays <= :days')
//                            ->setParameter("days", $noDays);
...
//                endif;
                ...
                if ($data['from'] && $data['to']):
                    $qb->andWhere('ic.dateFrom >= :startDate')
                            ->setParameter("startDate", $data['from']);
                    $qb->andWhere('ic.dateTo >= :startDate')
                            ->setParameter("startDate", $data['from']);
                    $qb->andWhere('ic.dateFrom <= :endDate')
                            ->setParameter("endDate", $data['to']);
...
//                    }
    ...
    protected function getRoomsCountAvailable($roomId, $start, $end, $type) {
        ...
        WHERE reservation_resort_room.room_id = $roomId AND reservation.is_booked = 1 AND reservation.is_deleted = 0 AND reservation.status IN(1,2)
        AND '$start' BETWEEN reservation.date_from
        AND reservation.date_to
        OR reservation.date_to
        BETWEEN '$start'
        AND '$end' AND reservation_resort_room.room_id = $roomId AND reservation.is_booked = 1 AND reservation.is_deleted = 0 AND reservation.status IN(1,2)
        OR '$end'
        BETWEEN reservation.date_from
        AND reservation.date_to AND reservation_resort_room.room_id = $roomId AND reservation.is_booked = 1 AND reservation.is_deleted = 0 AND reservation.status IN(1,2)
        OR reservation.date_from
        BETWEEN '$start'
        AND '$end'
        AND reservation_resort_room.room_id = $roomId AND reservation.is_booked = 1 AND reservation.is_deleted = 0 AND reservation.status IN(1,2)", $rsm);
                break;
        ...
        WHERE reservation_event_room.event_room_id = $roomId AND reservation.is_booked = 1 AND reservation.is_deleted = 0 AND reservation.status IN(1,2)", $rsm);
//        AND '$start' BETWEEN reservation.date_from
//        AND reservation.date_to
//        OR reservation.date_to
//        BETWEEN '$start'
...
//        AND reservation_event_room.event_room_id = $roomId AND reservation.is_booked = 1 AND reservation.status IN(1,2)", $rsm);
        ...
        WHERE reservation_cruise_cabin.cabin_id = $roomId AND reservation.is_booked = 1 AND reservation.is_deleted = 0 AND reservation.status IN(1,2)
        AND '$start' BETWEEN reservation.date_from
        AND reservation.date_to
        OR reservation.date_to
        BETWEEN '$start'
        AND '$end' AND reservation_cruise_cabin.cabin_id = $roomId AND reservation.is_booked = 1 AND reservation.is_deleted = 0 AND reservation.status IN(1,2)
        OR '$end'
        BETWEEN reservation.date_from
        AND reservation.date_to AND reservation_cruise_cabin.cabin_id = $roomId AND reservation.is_booked = 1 AND reservation.is_deleted = 0 AND reservation.status IN(1,2)
        OR reservation.date_from
        BETWEEN '$start'
        AND '$end'
        AND reservation_cruise_cabin.cabin_id = $roomId AND reservation.is_booked = 1 AND reservation.is_deleted = 0 AND reservation.status IN(1,2)", $rsm);
                break;
        }
...
//        d($result);
    ...
    public function getCollectionPending($startdate = null, $enddate = null, $page = false) {
        ...
        if (!empty($startdate) || !empty($enddate)) {
            ...
            $qb->select('c')->from('\Base\Entity\Reservation', 'c')
                    ->where($qury)
                    ->andwhere('c.isBooked=0 AND c.status!=3 AND c.isDraft=0 AND c.isDeleted=0')
                    ->orderBy('c.id', 'DESC');
            // ->setParameter('id', $startdate)
            // ->setParameter('endid', $enddate);
            ...
            $qb = $em->createQueryBuilder();
            $qb->select('c')->from('\Base\Entity\Reservation', 'c')
                    ->where('c.isBooked=0 AND c.status!=3 AND c.isDraft=0 AND c.isDeleted=0')
                    ->orderBy('c.id', 'DESC');
    ...
    public function reinstateCancelledReservation($id) {
        ...
        if ($item) {
            if ((($item->getFinalCost() + 1) >= $item->getDepositAmoun()) && (($item->getFinalCost() - 1) <= $item->getDepositAmoun())) {
...
//        $maxFinalPaymentDue = $this->subtractDaysFromDates($dateStart,45);
    ...
    public function updateExtraPaymentStatus($rId = NULL) {
        if ($rId == NULL || empty($rId)) {
            $where = 'c.status != 0';
        } else {
            $where = 'c.id=' . $rId;
        }
        $em = $this->getEntityManager();
        $qb = $em->createQueryBuilder();
        $qb->select('c')->from('\Base\Entity\Reservation', 'c')->where($where.' AND c.isDeleted = 0');
        ...
        foreach ($collection as $row) {
            if ($row->getIsUpdated() == 1) {
                ...
                } elseif ($row->getIsRefund() == 1 && $row->getRefundAmt() != 0.00) {
                    $em = $this->getEntityManager();
    ...
    public function updateDepositDueAmounts($rId = NULL) {
        if ($rId == NULL || empty($rId)) {
            $where = 'c.status != 0 AND c.isExtraPaid=0';
        } else {
            $where = 'c.id=' . $rId . ' AND c.isExtraPaid=0';
        }
        $em = $this->getEntityManager();
        $qb = $em->createQueryBuilder();
        ...
        $qb1->select('c')->from('\Base\Entity\Reservation', 'c')->where($where .' AND c.isDeleted = 0');
        $query = $qb1->getQuery();
        $collectionres = $query->getResult();

        foreach ($collectionres as $row) {
            if ($row->getIsUpdated() == 1) {
    ...
    public function updateReservationStatus() {
        $em = $this->getEntityManager();
        $qb = $em->createQueryBuilder();
        $qb->select('c')->from('\Base\Entity\Reservation', 'c')->where('c.status != 0 AND c.isDeleted = 0');
    ...
    public function getDraftCollection($startdate = null, $enddate = null, $page = false) {
        ...
        if (!empty($startdate) || !empty($enddate)) {
            ...
            If (!empty($startdate) && !empty($enddate)) {
                $qury = "c.createdAt >= '$startdate' AND c.createdAt <= '$enddate' AND c.isDraft=1  AND c.isDeleted = 0";
            } else if (!empty($startdate) && empty($enddate)) {

                $qury = "c.createdAt >= '$startdate' AND c.isDraft=1 AND c.isDeleted = 0";
            } else if (empty($startdate) && !empty($enddate)) {

                $qury = "c.createdAt <= '$enddate' AND c.isDraft=1 AND c.isDeleted = 0";
            }
    ...
    function delete($id){
        $em = $this->getEntityManager();
        $reservationEntity = $em->find('\Base\Entity\Reservation', (int) $id);
        
        if (!empty($reservationEntity)) {
