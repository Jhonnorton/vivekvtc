...
//         d($depExcAmt);
    ...
    public function updateExtraPaymentStatus($rId){
        $em = $this->getEntityManager();
            $qb = $em->createQueryBuilder();
            $qb->select('c')->from('\Base\Entity\Reservation', 'c')
                    ->where('c.id='.$rId);
            ...
            if($collection['0']->getIsUpdated()==1){
                if($collection['0']->getIsExtraPaid()==0 && $collection['0']->getExtraAmtPaid()!=0.00 && $collection['0']->getRefundAmt()==0.00){
                   $newDepamt= ($collection['0']->getDepositAmoun())-($collection['0']->getExtraDepAmt());
                   
                   $em = $this->getEntityManager();

                    $entity = $em->getRepository('\Base\Entity\Reservation')->findBy(array('id' => $rId));
                    if(!empty($entity)){
                        $entity = $entity[0];
